...
function listFunc(){
	...
	if(result){
		...
		$.ajax({
			url:"<%=request.getContextPath()%>/lecture/cancel.do",
				dataType : "json",
				data : {'id':data_id},
				success : function(obj) {
					...
					case 0:
						...
						break;
					case -1:
						alert('추천취소중에 오류가 발생했습니다. 잠시후 다시 시도 해주세요.')
						break;
					}
				}
			}); 
...
<div style="width: 1000px; height: 100%; text-align: center;">
	...
	<div style="border:2px solid gray; width:1000px; height:500px; margin-top: 20px;overflow: scroll;" >
...
for(LectureBean bean:list){
	...
	out.print("<tr style='border: 1px solid navy'>");
	   //for(int a=0; a>6; a++){
		   ...
		   out.print("<td><input type=button value=추천취소 class=c_cancel_button id=cancel_"+bean.getId()+" data_id="+bean.getId()+"></td>");
